...
const typeColors: Record<string, string> = {
  'Full-time': 'text-blue-400 bg-blue-500/10',
  'Part-time': 'text-violet-400 bg-violet-500/10',
  'Contract': 'text-amber-400 bg-amber-500/10',
  'Internship': 'text-green-400 bg-green-500/10',
}
    ...
    return (
      <div>
        ...
        <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] text-center">
          ...
          <div className="text-sm font-semibold text-white mb-2">Connect your wallet to manage hiring</div>
          <p className="text-xs text-white/40 max-w-xs">Job postings are stored securely linked to your wallet address.</p>
        </div>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Open Roles', value: openJobs.toString(), icon: Briefcase, color: 'text-blue-400', bg: 'from-blue-500/10 to-blue-500/5' },
            { label: 'Total Applicants', value: totalApplicants.toString(), icon: Users, color: 'text-violet-400', bg: 'from-violet-500/10 to-violet-500/5' },
            { label: 'Total Positions', value: jobs.length.toString(), icon: Globe, color: 'text-amber-400', bg: 'from-amber-500/10 to-amber-500/5' },
            { label: 'Hired This Month', value: '0', icon: CheckCircle, color: 'text-green-400', bg: 'from-green-500/10 to-green-500/5' },
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 glass rounded-xl border border-white/[0.06] p-1 mb-6 w-fit">
          {(['jobs', 'applicants'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-all capitalize ${
                activeTab === tab ? 'gradient-bg-primary text-white' : 'text-white/40 hover:text-white'
              }`}
            >
        ...
        {activeTab === 'jobs' && (
          loading ? (
            ...
            <div className="glass rounded-2xl border border-white/[0.06] p-12 flex flex-col items-center justify-center text-center">
              ...
              <div className="text-sm font-semibold text-white mb-2">No open positions</div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {jobs.map((job, i) => (
                <motion.div key={job.id}
                  ...
                  className="glass rounded-2xl border border-white/[0.06] p-5 hover:border-white/[0.12] transition-all group relative">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-white/40 mt-0.5">{job.department}</div>
                    </div>
                    <div className="relative">
                      <button
                        onClick={() => setMenuOpen(menuOpen === job.id ? null : job.id)}
                        className="w-7 h-7 rounded-lg glass border border-white/[0.08] opacity-0 group-hover:opacity-100 flex items-center justify-center text-white/40 hover:text-white transition-all flex-shrink-0">
                          ...
                          <div className="absolute right-0 top-8 w-40 glass-strong rounded-xl border border-white/[0.08] shadow-card overflow-hidden z-20">
                            ...
                            {job.status === 'open' && (
                              <button onClick={() => { updateJob(job.id, { status: 'closed' }); setMenuOpen(null) }}
                                className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-amber-400 hover:bg-amber-500/5 transition-colors text-left">
                  ...
                  {job.description && (
                    <p className="text-xs text-white/40 leading-relaxed line-clamp-2 mb-3">{job.description}</p>
                  )}
        ...
        {activeTab === 'applicants' && (
          <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <div className="text-sm text-white/30">No applicants yet</div>
              <div className="text-xs text-white/20 mt-1">Post a job to start receiving applications</div>
            </div>
          </div>
        )}
      </div>
