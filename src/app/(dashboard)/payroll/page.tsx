...
export default function PayrollPage() {
  ...
  const StatusIcon = ({ id }: { id: string }) => {
    const s = txStatus[id]
    if (s === 'pending') return <Clock className="w-4 h-4 text-white/30 animate-pulse" />
    if (s === 'sending') return <Loader2 className="w-4 h-4 text-indigo-400 animate-spin" />
    if (s === 'done') return <Check className="w-4 h-4 text-green-400" />
    if (s === 'error') return <X className="w-4 h-4 text-red-400" />
    if (s === 'no-wallet') return <AlertCircle className="w-4 h-4 text-amber-400" />
    return null
  }
  ...
  return (
    <div>
      ...
      <div className="p-8">
        ...
        {!isConnected && (
          <div className="mb-6 flex items-center gap-3 px-4 py-3.5 rounded-xl glass border border-amber-500/20 bg-amber-500/5">
            <AlertCircle className="w-4 h-4 text-amber-400 flex-shrink-0" />
            <span className="text-sm text-amber-300">Connect your wallet to execute payroll — USDC transfers require a connected wallet on Arc Testnet.</span>
          </div>
        )}
        {isConnected && !isOnArcTestnet && (
          <div className="mb-6 flex items-center gap-3 px-4 py-3.5 rounded-xl glass border border-red-500/20 bg-red-500/5">
            <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0" />
            <span className="text-sm text-red-300">Switch to Arc Network Testnet in your wallet to run payroll.</span>
          </div>
        )}

        {/* Tabs */}
        <div className="flex items-center gap-1 glass rounded-xl border border-white/[0.06] p-1 mb-8 w-fit">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                { label: 'Wallet Balance', value: isConnected ? `${formattedUSDC} USDC` : '—', icon: DollarSign, color: 'text-green-400', bg: 'from-green-500/10 to-green-500/5', sub: isConnected ? 'Arc Testnet' : 'Connect wallet' },
                { label: 'Total Monthly', value: totalMonthly > 0 ? mask(totalMonthly) : '$0', icon: Calendar, color: 'text-blue-400', bg: 'from-blue-500/10 to-blue-500/5', sub: 'USDC required' },
                { label: 'Active Employees', value: activeEmployees.length.toString(), icon: Users, color: 'text-violet-400', bg: 'from-violet-500/10 to-violet-500/5', sub: activeEmployees.length > 0 ? 'Ready for payroll' : 'No employees' },
                { label: 'Network Fees', value: '~$0.01', icon: Zap, color: 'text-amber-400', bg: 'from-amber-500/10 to-amber-500/5', sub: 'Arc Network low fees' },
              ].map(({ label, value, icon: Icon, color, bg, sub }) => (
                <div key={label} className={`glass rounded-xl border border-white/[0.06] p-5 bg-gradient-to-b ${bg}`}>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-3 bg-white/[0.05]">
            ...
            <div className="glass rounded-2xl border border-white/[0.06] p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h3 className="text-base font-semibold text-white">Execute Payroll Run</h3>
                  <p className="text-sm text-white/40 mt-0.5">Select employees and send USDC directly to their wallets</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Shield className={`w-4 h-4 ${confidential ? 'text-indigo-400' : 'text-white/30'}`} />
                    <span className="text-xs text-white/50">Confidential</span>
                    <button
                      onClick={() => setConfidential(!confidential)}
                      className={`relative w-10 rounded-full transition-all ${confidential ? 'bg-indigo-600' : 'bg-white/10'}`}
                      style={{ height: '22px' }}
                    >
              ...
              {activeEmployees.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed border-white/[0.08] rounded-xl">
                  <CreditCard className="w-10 h-10 text-white/10 mb-4" />
                  <div className="text-sm font-semibold text-white mb-2">No active employees</div>
                  <p className="text-xs text-white/40 max-w-xs mb-5">
                ...
                <>
                  ...
                  <div className="border border-white/[0.06] rounded-xl overflow-hidden mb-5">
                    <div className="flex items-center gap-4 px-4 py-3 border-b border-white/[0.05] bg-white/[0.02]">
                      <button onClick={selectAll} className="text-white/40 hover:text-white transition-colors">
                        {selected.size === activeEmployees.length && activeEmployees.length > 0
                          ? <CheckSquare className="w-4 h-4 text-indigo-400" />
                          : <Square className="w-4 h-4" />}
                      </button>
                      <span className="text-[10px] font-semibold text-white/30 uppercase tracking-widest flex-1">Employee</span>
                      <span className="text-[10px] font-semibold text-white/30 uppercase tracking-widest w-32">Wallet</span>
                      <span className="text-[10px] font-semibold text-white/30 uppercase tracking-widest w-28">Amount (USDC)</span>
                      <span className="text-[10px] font-semibold text-white/30 uppercase tracking-widest w-20">Status</span>
                    </div>
                    <div className="divide-y divide-white/[0.04]">
                      {activeEmployees.map(emp => (
                        <div key={emp.id} className="flex items-center gap-4 px-4 py-3.5 hover:bg-white/[0.02] transition-colors">
                          <button onClick={() => toggleSelect(emp.id)} className="text-white/40 hover:text-white transition-colors flex-shrink-0">
                            {selected.has(emp.id)
                              ? <CheckSquare className="w-4 h-4 text-indigo-400" />
                              : <Square className="w-4 h-4" />}
                          </button>
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-white/40 truncate">{emp.role}</div>
                            </div>
                          </div>
                          <div className="w-32">
                            {emp.wallet
                              ? <span className="text-xs font-mono text-white/50">{emp.wallet.slice(0, 8)}...{emp.wallet.slice(-4)}</span>
                              : <span className="text-xs text-amber-400">No wallet</span>}
                          </div>
                          <div className={`w-28 text-sm font-semibold ${revealed ? 'text-white' : 'text-indigo-300 tracking-widest'}`}>{mask(emp.salary)}</div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-white/50">
                      ...
                      <span className={`font-semibold ${revealed ? 'text-white' : 'text-indigo-300'}`}>{revealed ? `$${totalSelected.toLocaleString()} USDC` : '•••••• USDC'}</span> total
                    </div>
                    <button
              ...
              {confidential && (
                <div className="mt-5 flex items-center gap-2 px-4 py-3 rounded-xl bg-indigo-500/8 border border-indigo-500/15">
                  <Shield className="w-3.5 h-3.5 text-indigo-400 flex-shrink-0" />
                  <span className="text-xs text-indigo-300">Confidential mode enabled — Arcium MPC will encrypt salary data during execution</span>
                </div>
              )}
            </div>
            ...
            <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.05]">
                <h3 className="text-sm font-semibold text-white">Payroll History</h3>
              </div>
              {payrollRuns.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-14 text-center">
                  <Clock className="w-8 h-8 text-white/10 mb-3" />
                  <div className="text-sm text-white/30">No payroll runs yet</div>
                  <div className="text-xs text-white/20 mt-1">Completed payroll transactions will appear here</div>
                </div>
              ) : (
                <div className="divide-y divide-white/[0.04]">
                  {payrollRuns.map(run => (
                    <div key={run.id} className="px-5 py-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-right">
                          <div className={`text-sm font-semibold ${revealed ? 'text-white' : 'text-indigo-300'}`}>{mask(run.totalPaid)} USDC paid</div>
                      ...
                      <div className="space-y-1.5">
                        {run.transactions.map(tx => (
                          <div key={tx.employeeId} className="flex items-center justify-between text-xs text-white/40 pl-5">
                            <span>{tx.employeeName}</span>
                            <div className="flex items-center gap-3">
                              <span className={revealed ? '' : 'text-indigo-300'}>{mask(tx.amount)}</span>
                              {tx.status === 'success' && tx.txHash ? (
                                ...
                                <span className="text-red-400">{tx.error || 'Failed'}</span>
                              )}
                            </div>
                          </div>
                        ))}
        ...
        {activeTab === 'batch' && (
          <div className="space-y-6">
            <div className="glass rounded-2xl border border-white/[0.06] p-8 text-center">
              ...
              <div className="border-2 border-dashed border-white/[0.08] rounded-xl p-8 mb-6 hover:border-indigo-500/30 transition-colors cursor-pointer">
                <div className="text-sm text-white/30">Drop CSV file here or <span className="text-indigo-400">browse files</span></div>
                <div className="text-[10px] text-white/20 mt-1">Columns: name, wallet_address, amount_usdc · Max 500 rows</div>
              </div>
              <div className="flex items-center gap-3 justify-center">
                <button onClick={handleDownloadTemplate} className="flex items-center gap-2 px-5 py-2.5 rounded-xl glass border border-white/[0.1] text-sm text-white/60 hover:text-white transition-all">
        ...
        {activeTab === 'schedule' && (
          <div className="space-y-6">
            <div className="glass rounded-2xl border border-white/[0.06] p-12 text-center">
              ...
              <div className="text-sm font-semibold text-white mb-2">Automated Payroll Schedules</div>
              ...
              <div className="grid grid-cols-3 gap-4 max-w-md mx-auto mb-6">
                {['Weekly', 'Bi-weekly', 'Monthly'].map(s => (
                  <div key={s} className="glass rounded-xl border border-white/[0.06] p-4 text-center cursor-pointer hover:border-indigo-500/30 transition-all">
                    <div className="text-sm font-semibold text-white mb-1">{s}</div>
                    <div className="text-[10px] text-white/40">{s === 'Weekly' ? '52×/year' : s === 'Bi-weekly' ? '26×/year' : '12×/year'}</div>
                  </div>
                ))}
              </div>
