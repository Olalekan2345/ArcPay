      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 flex-shrink-0">
            <div>
              <h2 className="text-base font-semibold text-slate-900">{initial ? 'Edit Job' : 'Post a Job'}</h2>
              <p className="text-xs text-slate-400 mt-0.5">Add a new position to your hiring pipeline</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input value={form.title} onChange={e => set('title', e.target.value)}
                    ...
                    className={`w-full bg-surface-50 border rounded-xl pl-9 pr-3 py-2.5 text-sm text-slate-900 placeholder-slate-300 outline-none transition-all ${errors.title ? 'border-red-400' : 'border-slate-200 focus:border-brand-400'}`} />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select value={form.department} onChange={e => set('department', e.target.value)}
                      className={`w-full bg-surface-50 border rounded-xl pl-9 pr-3 py-2.5 text-sm text-slate-900 outline-none transition-all appearance-none ${errors.department ? 'border-red-400' : 'border-slate-200 focus:border-brand-400'}`}>
                ...
                <div>
                  ...
                  <select value={form.type} onChange={e => set('type', e.target.value)}
                    className="w-full bg-surface-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-brand-400 transition-all appearance-none">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input value={form.location} onChange={e => set('location', e.target.value)}
                    ...
                    className={`w-full bg-surface-50 border rounded-xl pl-9 pr-3 py-2.5 text-sm text-slate-900 placeholder-slate-300 outline-none transition-all ${errors.location ? 'border-red-400' : 'border-slate-200 focus:border-brand-400'}`} />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1.5">Min Salary (USDC/mo)</label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                    <input value={form.salaryMin} onChange={e => set('salaryMin', e.target.value)}
                      placeholder="3000" type="number" min="0"
                      className={`w-full bg-surface-50 border rounded-xl pl-9 pr-3 py-2.5 text-sm text-slate-900 placeholder-slate-300 outline-none transition-all ${errors.salaryMin ? 'border-red-400' : 'border-slate-200 focus:border-brand-400'}`} />
                  </div>
                  {errors.salaryMin && <p className="text-[11px] text-red-500 mt-1">{errors.salaryMin}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1.5">Max Salary (USDC/mo)</label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                    <input value={form.salaryMax} onChange={e => set('salaryMax', e.target.value)}
                      placeholder="6000" type="number" min="0"
                      className={`w-full bg-surface-50 border rounded-xl pl-9 pr-3 py-2.5 text-sm text-slate-900 placeholder-slate-300 outline-none transition-all ${errors.salaryMax ? 'border-red-400' : 'border-slate-200 focus:border-brand-400'}`} />
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Job Description</label>
                <div className="relative">
                  ...
                  <textarea value={form.description} onChange={e => set('description', e.target.value)}
                    ...
                    rows={3}
                    className="w-full bg-surface-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2.5 text-sm text-slate-900 placeholder-slate-300 outline-none focus:border-brand-400 transition-all resize-none" />
                </div>
              </div>
